...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", relev_categories=None):
    ...
    filters = {
        "terms": {
            "categoryPathIds": relev_categories
        }
    }
...
if __name__ == "__main__":
    ...
    for line in fileinput.input():
        ...
        #predict categories and include in search query filter
        relev_categories = []
        total_score = 0
        ret_cats = q_classifier.predict(query, k=5)
        print(ret_cats)
        for idx, cat in enumerate(ret_cats[0]):
            relev_categories.append(cat.replace("__label__", ""))
            total_score = total_score + ret_cats[1][idx]
            if total_score > 0.5:
                break

        print(relev_categories)
        search(client=opensearch, user_query=query, index=index_name, relev_categories=relev_categories)
