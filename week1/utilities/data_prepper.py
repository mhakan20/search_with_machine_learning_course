...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        for hit in hits:
            ...
            #capture all feature keys 
            feature_keys = hit['fields']['_ltrlog'][0]['log_entry']
            # for key in feature_keys:
            #     print(key)
            #append entries to each feature
            for key in feature_keys:
                feature_name = key['name']
                feature_value = key.get('value', 0)
        ...
        #     print(feature)
        #     print(len(feature_results[feature])) 
        df_features = pd.DataFrame(feature_results)

        return df_features.astype({'doc_id': 'int64', 'query_id': 'int64', 'sku': 'int64'})
